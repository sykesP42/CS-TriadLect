...
> **第一次用、没碰过命令行？** 从 **[docs/00-新手使用说明](docs/00-新手使用说明.md)** 开始 ——
> 它从装 Git 和 C++ 编译器讲起，一步不跳，每一步都写清楚你会看到什么。
...
| `W` `A` `S` `D` | 走 |
| 鼠标 | 转视角 |
| `E` | 和面前的东西交互。**控制台就是在终端上按 E 打开的** —— 没有快捷键 |
| `R` | **重新读 `content/` 里的数据 —— 世界当场变，不重启** |
| `F2` | 存一张截图到 `shots/` |
| `Q` | 看**入群二维码** —— 四关全通之后才解锁，之前按只会提示还没开 |
| `Esc` | 关控制台；都关着时 = **打开暂停菜单**。退出也在菜单里 —— 按 Esc 不会直接退 |

> 控制台**不是**一个随时能叫出来的调试窗口 —— 它是桌子上的那台终端。
> 走过去按 `E` 才打得开。这是故意的：这个游戏的核心循环就是
> "走到终端前按 E → 它告诉你该改哪个文件"，控制台要是能随手叫出来，这一步就没了。

## 暂停菜单

按 `Esc` 打开，里面能改画面、看进度、退出游戏。
...
```
       ...
       ────────────────────────────
        第 1 关 · 第一束光
        目标：把主光装回吊灯里并点亮，让展台亮起来    ████░░░░  35%
       ────────────────────────────
        退出游戏
        按 Esc 回到游戏
```

**分辨率上限是 720p**，只有三档（854×480 / 960×540 / 1280×720）。两个理由：
这是个**纯 CPU 的软光栅化器**，再往上帧率撑不住；以及全屏走的是"用你选的
分辨率渲染、再拉伸铺满屏幕"，所以档位必须是 16:9，不然会被拉变形。

拉伸用的是**最近邻**——画面放大之后像素是整齐的方块，不是糊的。顺带一提，
在 2560×1440 的屏幕上 720p 正好是 **2.00 倍**、480p 是 **3.00 倍**，
每块像素一样大（540p 是 2.667 倍，会有的宽有的窄）。

**窗口模式**：窗口 / 无边框（没有标题栏，按所选分辨率居中）/ 全屏（铺满显示器）。
三种都只改窗口样式、不重建窗口，所以切换是瞬间的，不会闪。

设置会**记住**，存在 `saved/settings.txt` —— 和存档是两个文件，
所以 `--reset`（清进度）不会顺手把你调好的窗口尺寸也清了。

窗口默认按你的核数自适应（4 核及以下 960×540、8 核及以上 1280×720），**上限 720p** ——
理由见下面的「暂停菜单」。
...
四关全打完，屏幕正中会弹出**数媒组的招新群二维码**；以后按 `Q` 随时能再看。

这张码在仓库里是**混淆过的**（`assets/recruit/qr.bin`，一格一个 bit），不是加密 ——
密钥就写在 `engine/recruit.h` 里，谁真想解都能解。它挡的只是"在 GitHub 上点开文件
扫一下"，真正的门槛是"你得先把四关打完"。**这是游戏设计，不是安全边界。**

> ### 维护者：二维码会过期
>
> 微信的群二维码有有效期。过期之后游戏里显示的还是一张码，但**扫不出来** ——
> 它不会报错，只会安静地失效。所以每次招新前换一张新的：
>
> ```bash
> python tools/gen_recruit_qr.py --src <新的二维码图>
> build\dreamlab.exe --selftest          # 确认三个定位角还对得上
> ```
>
> 脚本自己从定位角量出格网和模块数，**换图直接重跑，不用改任何代码**。
> 想先看一眼画出来什么样：`build\dreamlab.exe --qr-preview --shot build\qr.png`。
>
> **源图不要放进仓库。** 原始二维码图进了公开仓库，混淆就白做了 —— 它只留在
> 维护者本地，仓库里只有混淆后的 `.bin`。
...
**走到桌子前的终端，按 `E`。** 命令全是 ASCII，不用切输入法：
...
| 尺寸 | 4 线程 | 8 线程 | 32 核 |
|---|---|---|---|
| 854×480 | 41 帧 | 59 帧 | 61 帧 |
| 960×540 | 36 帧 | 56 帧 | 57 帧 |
| 1280×720 | 19 帧 | 37 帧 | 46 帧 |

（这三档就是菜单里能选的全部，封顶 720p。**帧率对画面内容很敏感** ——
同一个尺寸，站在空走廊和站在满屏地砖的屋子前能差一倍。）
